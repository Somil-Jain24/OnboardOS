...
import { Sparkles, ArrowRight, ShieldCheck, Lock, Mail, Eye, EyeOff, AlertCircle } from 'lucide-react';
...
export function LoginPage() {
  ...
  const handleQuickFill = (demoUser: (typeof SEEDED_DEMO_USERS)[0]) => {
    setEmail(demoUser.email);
    setPassword(demoUser.devPasswordHint);
    setError(null);
  };
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="name@onboardos.internal"
                  required
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs text-slate-900 placeholder:text-slate-400 focus:bg-white focus:border-blue-500 focus:outline-none transition-colors"
                />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="••••••••••••"
                  required
                  className="w-full pl-10 pr-10 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs text-slate-900 placeholder:text-slate-400 focus:bg-white focus:border-blue-500 focus:outline-none transition-colors"
                />
                <button
                  type="button"
            ...
            <div className="flex flex-wrap gap-1.5">
              {SEEDED_DEMO_USERS.map((u) => (
                <button
                  ...
                  className="px-2.5 py-1 rounded-xl bg-slate-100 hover:bg-blue-50 hover:text-blue-700 hover:border-blue-200 border border-slate-200 text-[11px] font-medium text-slate-600 transition-colors"
                >
                  {u.name} ({u.role})
                </button>
              ))}
