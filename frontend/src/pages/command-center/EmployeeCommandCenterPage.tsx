...
export function EmployeeCommandCenterPage() {
  ...
  return (
    <div className="space-y-6 text-left">
      ...
      {/* Automation & AI Decision Intelligence Card */}
      <div className="p-6 bg-gradient-to-r from-blue-50/70 via-indigo-50/50 to-purple-50/70 border border-blue-200/90 rounded-3xl shadow-card space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-blue-100/90 pb-3.5">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                Live Automation & Decision Intelligence
                <Badge variant="info" size="sm">Active Outbox</Badge>
              </h3>
              <p className="text-xs text-slate-500">
                Deterministic DAG state machine + AI reasoning grounded in live policy rules.
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-3.5 bg-white/90 border border-blue-100 rounded-2xl space-y-1">
            <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider font-mono flex items-center gap-1">
              <Zap className="w-3 h-3 text-indigo-600" />
              Latest Automation Dispatch
            </div>
            <p className="text-xs font-semibold text-slate-800">
              ViaSocket Webhook Dispatched
            </p>
            <p className="text-[11px] text-slate-500">
              HR & IT Slack alerts sent + Google Sheet tracking row created.
            </p>
          </div>
          ...
          <div className="p-3.5 bg-white/90 border border-blue-100 rounded-2xl space-y-1">
            ...
            <p className="text-xs font-semibold text-slate-800">
              {tasks.some((t) => t.status === 'FAILED')
                ? 'Retry Jira SCIM Connector'
                : tasks.some((t) => t.status === 'WAITING_APPROVAL')
                ? 'Awaiting Marcus Vance Approval'
                : 'Complete Day-1 Training Checklist'}
            </p>
            <p className="text-[11px] text-slate-500">
              {tasks.some((t) => t.status === 'FAILED')
                ? 'HTTP 503 external rate limit holding 1 dependent task.'
                : tasks.some((t) => t.status === 'WAITING_APPROVAL')
                ? 'AWS Cloud IAM gated by Least Privilege Policy v1.2.'
                : '100% Day-1 prerequisites satisfied.'}
            </p>
          </div>
          ...
          <div className="p-3.5 bg-white/90 border border-blue-100 rounded-2xl space-y-1">
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm font-extrabold text-slate-900">{readinessScore}%</span>
              <Badge variant={readinessScore >= 90 ? 'success' : 'warning'} size="sm">
                {readinessScore >= 90 ? 'READY' : 'AT RISK'}
              </Badge>
            </div>
            <p className="text-[11px] text-slate-500">
              {readinessScore >= 90
                ? 'All mandatory access & keys verified.'
                : 'Blocker flag active. 1-click recovery available.'}
            </p>
          </div>
        </div>
      </div>
